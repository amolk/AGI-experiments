...
class DynamicsExperiment1:
    def __init__(
        self,
        image_shape: Tuple = (71, 71),
        patch_grid_shape=(31, 31),
    ...
    def run(self, epochs=50):
        ...
        for epoch in range(epochs):
            for t in range(1, t_end):
                ...
                # td_mu
                image = self.layer.td_input.components["mu"].pixels.view(mu_shape).numpy().copy()
                image = resize_image(image, p_mu_image.shape, interpolation=cv2.INTER_NEAREST)
                images.append(image)
                images_vmin.append(0)
                images_vmax.append(1)
                ...
                image = resize_image(image, p_mu_image.shape, interpolation=cv2.INTER_NEAREST)
                images.append(image)
                images_vmin.append(0.0)
                images_vmax.append(1.0)
...
for img_index in [5, 7]:
    filenames = []
    for i in range(img_index, len(experiment.images), 8):
    ...
    with imageio.get_writer(
        f"./06_multi_patch_high_d_temporal_to_spatial_{img_index}.gif", mode="I"
    ) as writer:
