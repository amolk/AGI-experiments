...
class DynamicsExperiment1:
    def __init__(
        ...
        patch_grid_shape=(9, 9),
        per_patch_pattern_grid_shape: Tuple = (5, 5),
    ):
        ...
        self.signal_source = VideoSource(
            path="/Users/amolk/work/AGI/pattern-machine/notebooks/08_cat_small.m4v",
    ...
    def run(self, epochs=5, debug=True, learn=True, hallucinate_after=None):
        ...
        images_vmin = []
        images_vmax = []
        t_end = self.signal_source.item_count
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for t in range(1, t_end):
                ...
                if hallucinate_after is not None:
                    if t > hallucinate_after:
                        raw_input_image.components["mu"].precision.data *= 0
                        raw_input_image.components["mu"].precision.data += MIN_PRECISION
                        trace_input = None
                ...
                self.layer.forward(trace_input=trace_input, learn=learn)
...
## TRAIN
# experiment.run(epochs=100, debug=False)
# with open("08.patterns.pkl", "wb") as fp:
#     pickle.dump(experiment.layer.patterns, fp)
# experiment.run(epochs=1, debug=True)

## LOAD MODEL
with open("08.patterns.pkl", "rb") as fp:
    experiment.layer.patterns = pickle.load(fp)
experiment.run(epochs=1, debug=True, learn=False, hallucinate_after=None)
...
for img_index in [8]:  # [1, 8, 10]:
    ...
    # Build GIF
    with imageio.get_writer(f"./08_{img_index}.gif", mode="I") as writer:
...
# %lprun -D prof experiment.run(epochs=20, debug=False)
experiment.run(epochs=1, debug=True, learn=False, hallucinate_after=None)
