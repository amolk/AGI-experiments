...
def pretty_s(name, klass, indent=0):
    if type(klass).__name__ in ["Tensor", "Trace"]:
        return " " * indent + name + ":" + type(klass).__name__ + " size" + str(tuple(klass.shape))
...
def plot_patterns(
    patterns, pattern_lr, dataset, voronoi=False, annotate=False, figsize=(7, 7), dpi=100
):
...
def make_kernel_grid(
    ...
    if kernel_type == KERNEL_RANDOM or kernel_type is None:
        ...
        if kernel_type == KERNEL_GAUSSIAN:
            kernel_pixels = gaussian_kernel(l=min(signal_shape), sig=min(signal_shape) * 0.3)
        ...
        pixels = kernel_pixels_expanded.expand((grid_shape[0], kernel_pixels_expanded.shape[0]))

        if noise_amplitude > 0:
            noise_pixels = (
                torch.rand_like(pixels).to(device) * noise_amplitude - noise_amplitude * 0.5
            )
